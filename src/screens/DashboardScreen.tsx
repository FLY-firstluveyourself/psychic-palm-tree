    ...
    <div className="dashboard-screen">
      <div className="dashboard-container">
        ...
        <div className="security-features-section">
          <h3 className="section-title">🛡️ Security & Recovery</h3>
          <div className="features-grid">
            {onGuardians && (
              <button className="feature-btn" onClick={onGuardians}>
                <span className="feature-icon">👥</span>
                <span className="feature-label">Guardians</span>
                <span className="feature-desc">Manage recovery contacts</span>
              </button>
            )}
            {onSecurity && (
              <button className="feature-btn" onClick={onSecurity}>
                <span className="feature-icon">🔍</span>
                <span className="feature-label">Monitor</span>
                <span className="feature-desc">Track device access</span>
              </button>
            )}
            {onEmergency && (
              <button className="feature-btn emergency-btn" onClick={onEmergency}>
                <span className="feature-icon">🚨</span>
                <span className="feature-label">Emergency</span>
                <span className="feature-desc">Lock & kill-switch</span>
              </button>
            )}
            {onReport && (
              <button className="feature-btn report-btn" onClick={onReport}>
                <span className="feature-icon">🚔</span>
                <span className="feature-label">Report</span>
                <span className="feature-desc">Report compromised wallet</span>
              </button>
            )}
          </div>
        </div>
